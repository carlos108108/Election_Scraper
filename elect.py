...
def election_file(FILE: str, header: list, list_of_lists: list) -> csv:
    with open(FILE, 'w') as f:
        f_writer = csv.writer(f)
        f_writer.writerow(header)
        f_writer.writerows(list_of_lists)



def get_list_of_values(main_tuple: tuple) -> list:
    list_of_values = list()
    for i in range(len(main_tuple[0])):
        path = main_tuple[1][i]
        x1 = main_tuple[0][i]
...
def get_links(first_path: str) -> tuple:
    list_links = list()
    list_numbers = list()
    c1 = list()
    getr = requests.get(first_path)
